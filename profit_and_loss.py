...
#         print(f"[PROFIT DEFICIT] DAY: {deficit_day}, AMOUNT: USD{abs(int(deficit_amount))}")
    ...
    deficit_output = []  # Create an empty list to store deficit information

    # Print the profit deficit information from the list
    for result in aList:
        deficit_day, deficit_amount = result
        deficit_line = f"[PROFIT DEFICIT] DAY: {deficit_day}, AMOUNT: USD{abs(int(deficit_amount))}"
        deficit_output.append(deficit_line)  # Append deficit line to the output list
    
    return deficit_output  # Return the list of deficit information
